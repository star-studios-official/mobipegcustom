...
namespace Gericom.FastVideoDS
{
    public class FastVideoDSEncoder
    {
        ...
        // Scalar port of the AVX2 original: per-pixel sum of absolute differences
        // of the original against the I- and P-frame predictions.
        private static (ulong sadI, ulong sadP) SadIP(ReadOnlySpan<byte> original, ReadOnlySpan<byte> iFrame,
            ReadOnlySpan<byte> pFrame)
        {
            ulong sadI = 0, sadP = 0;
            for (int i = 0; i < original.Length; i++)
            {
                sadI += (ulong)Math.Abs(original[i] - iFrame[i]);
                sadP += (ulong)Math.Abs(original[i] - pFrame[i]);
            }

            return (sadI, sadP);
        }

        // Scalar port of the AVX2 original: average two 5-bit samples, biasing
        // each non-zero sample by +4 before the >>4<<3 requantise.
        private void GxAverage(ReadOnlySpan<byte> srcA, ReadOnlySpan<byte> srcB, Span<byte> dst)
        {
        ...
        private static bool AllZero(int[] src)
        {
