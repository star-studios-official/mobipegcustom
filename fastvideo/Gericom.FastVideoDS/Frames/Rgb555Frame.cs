...
namespace Gericom.FastVideoDS.Frames
{
    public class Rgb555Frame
    {
        ...
        {
            // src is BGRA byte order (ffmpeg -pix_fmt bgra), matching the original
            // which read an int and took Color.FromArgb: R=byte2, G=byte1, B=byte0.
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    byte* p = src + y * stride + x * 4;
                    int cR = p[2], cG = p[1], cB = p[0];

                    int bias = DitherMatrix[y & 3, x & 3];

                    int pr = Dither(cR, bias / 16.0);
                    int pg = Dither(cG, bias / 16.0);
                    int pb = Dither(cB, bias / 16.0);
